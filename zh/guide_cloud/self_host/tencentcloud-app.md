...
1. 首先需要一个腾讯云账号，并确保账号拥有 **安装云应用的权限**

2. 在 [腾讯云控制台-私有网络](https://console.cloud.tencent.com/vpc/vpc) 中，创建一个默认的 VPC，为云应用提供目标网络，
目前支持的地域如下：
...
各云资源代表的含义如下：
- 目标网络：即云服务托管地域，可以根据之前创建 `VPC` 的地域进行选择；
- 云服务器类型：即托管 `SwanLab` 自托管版的云服务器实例配置，最低可选配置为：
    - CPU: ≥ 4 核
    - 内存：≥ 8GB
    - 系统存储空间：默认 40GB
- 数据盘大小：即 `SwanLab` 自托管版用于记录实验数据的硬盘大小，默认为 `100GB`，最低 `40GB`
