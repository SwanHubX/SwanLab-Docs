...
在分布式训练中，训练任务通常会在多个进程、多个 GPU 甚至多台机器上同时运行。SwanLab 提供了以下方式來记录这些训练指标：

1. **仅记录主进程**：只记录 rank 0（主进程/协调进程）的指标
2. **多进程分别记录**：每个进程创建独立的运行，使用 `group` 参数将它们关联到同一个实验

> **注意**：SwanLab 不支持将所有进程记录到单个运行中。所有进程需要分别记录为独立的运行。
...
每个进程创建独立的运行，使用 `group` 参数将它们关联到同一个实验组，使用 `job_type` 参数区分不同类型的节点（如 "main" 和 "worker"）。
...
| 环境变量 | 描述 |
|---------|------|
| `SWANLAB_GROUP` | 实验分组名称，用于关联多个运行 |
| `SWANLAB_JOB_TYPE` | 任务类型，如 "train"、"eval"、"inference" |
| `SWANLAB_NAME` | 运行名称 |
| `SWANLAB_DESCRIPTION` | 运行描述 |
...
4. **适当结束运行**：在训练结束后调用 `swanlab.finish()` 确保数据正确上传
