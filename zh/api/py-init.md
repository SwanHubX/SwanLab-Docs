...
| mode            | (str) 设置swanlab实验创建的模式，可选"online"、"local"、"offline"、"disabled"，默认设置为"online"。<br>`online`：将实验上传到SwanLab云端。（公有云和私有化部署）<br>`offline`：仅将实验数据保存到本地。<br>`local`：不上传到云端，但会记录实验数据和一些可被`swanlab watch`打开的数据到本地。<br>`disabled`：不上传也不记录。                             |
...
| id              | (str) 上次实验的运行ID，用于恢复上次实验。支持1-64位自定义字符，不可含有 `/ \ # ? % :` 等特殊字符。                                                                                                                                                                                                                                                       |
...
| parallel        | (str) 并行模式，可选`"none"`、`"shared"`，默认为`"none"`。<br>`"shared"`：共享并行模式，多个分布式进程可同时上传指标到同一个实验。启用后自动强制 `mode='online'` 和 `resume='allow'`，若未提供 `id` 则自动生成一个8位随机ID。                                                                                                                             |
...
`parallel="shared"` 适用于多进程分布式训练场景，多个进程同时向同一实验上传指标：

```python
# 多个分布式进程使用相同的 id，指标将汇聚到同一个实验下
swanlab.init(
    parallel="shared",
    id="my-distributed-run",  # 所有进程使用相同的 id
)
```

若不指定 `id`，SwanLab 会自动生成一个8位随机ID：

```python
import os

# 通过环境变量在所有进程间共享同一个 id
...
swanlab.init(
    resume=True,
    id="14pk4qbyav4toobziszli",  # 支持1-64位自定义字符
)
```
