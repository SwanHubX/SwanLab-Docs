...
| 参数         | 描述                                                                                                                                                         |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| key          | 指标名，支持精确 key 或末尾带单个 `*` 的 glob 匹配（如 `"train/*"`）。`"*loss"`、`"train/*/x"`、`"train/**"` 等写法会被拒绝并报错；系统指标 key 不参与匹配。 |
...
| step_sync    | X 轴与 Y 轴指标分开 log 时，自动为 Y 值补上最近一次的 X 值，传入 `x_axis`/`step_metric` 时默认开启。                                                         |
...
需要注意：对同一个指标多次定义时，`hidden` 只要有一次设为 `True`，结果就是 `True`——之后的定义（包括 `overwrite=True`）都无法把它改回 `False`。
