...
![只预测最后回复](./example/multi-data-1.png)
...
<img src="./example/datas.png" alt="多轮对话数据集" style="width:75%;">
...
![deepseek模型微调输入模板](./example/deepseek_chat_template.png)
...
![r=4的时候alpha不同的对比结果](./example/r-alpha-vs.png)
...
![alpha不同的对比结果](./example/many-r-alpha-vs.png)
...
![正则化参数对实验的影响](./example/dropout.png)
...
![不同模型层的loss曲线变化](./example/moxingceng.png)

![实验时间对比](./example/time.png)
...
![batch设置不同的时候结果对比](./example/batch.png)
...
![batch不同的时候训练时长对比](./example/batch_time.png)
...
![具体实验硬件条件](./example/gpus_info.png)

---

![GPU的监测情况](./example/gpus.png)
