...
该架构图描述了集群**本地数据库部署方案**下， SwanLab 私有化服务的数据库进行跨集群数据迁移的流程，包含三个核心区域：
...
> ⚠️ **注意：数据迁移时需要确保两套 SwanLab 服务的 DB 和 S3 都保持停机状态**，在一个中间 Pod 中执行迁移任务（Job），不然会因为状态不一致等问题造成打包失败或数据混乱。
>
> **迁移前必须停机！**
