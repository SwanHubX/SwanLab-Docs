# 使用Kubernetes进行部署 <img src="/kubernetes.svg" class="deploy-logo" alt="Kubernetes" /> <VersionBadge version="v3.2.0" />
...
- 受不同 Kubernetes 集群权限模型的限制，SwanLab 的 K8s 版本暂未使用 Operator 部署，**基础数据库服务使用单实例部署**
- 如需接入自建或云厂商的高可用数据库，可参考 [自定义 value 配置](./configuration.md) 进行修改
- 外接数据库前，建议参考 [FAQ](./faq.md#【慢响应】如何测试集群与外部数据库之间的-rtt) 测试集群与数据库实例之间的 RTT
  :::
...
::: info
本指引使用 **【外部集成 S3 对象存储】 + 【集群数据库通过 PVC 挂载云硬盘】** 的部署方案。
:::
...
完整的 `value.yaml` 配置项可参考 [自定义 Value 配置说明](./configuration.md) 文档。
...
如果您希望**开启外部访问（通过IP或域名）**，请参考 [配置应用访问入口](./configuration.md#配置应用访问入口)。
...
详细的字段说明与配置实践，请参阅 [自定义 Value 配置说明](./configuration.md) 文档，其中涵盖：
