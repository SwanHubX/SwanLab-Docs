...
> This feature requires swanlab >= `0.10.0`.
...
All parameters after `key` are keyword-only.

| Parameter    | Description                                                                                                                                                                                                                                                                                                    |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| key          | Metric key. Supports an exact key or a glob with a single trailing `*` (e.g. `"train/*"`; a bare `"*"` matches all custom metrics). Patterns like `"*loss"`, `"train/*/x"` and `"train/**"` are rejected (an error is reported and the definition does not take effect); system metric keys are never matched. |
| x_axis       | Metric key used as the custom X axis. `None` uses the system step as the X axis. **The `step_metric` parameter is accepted as an alias**.                                                                                                                                                                      |
| section_name | Chart section (group) name. `None` uses the default grouping derived from the key prefix.                                                                                                                                                                                                                      |
| hidden       | Whether to hide the chart. `True` places the chart in the HIDDEN section; `False` explicitly unhides it. Defaults to `None` (not specified), which keeps the previous setting when merging.                                                                                                                    |
| step_sync    | When X-axis and Y-axis metrics are logged separately, whether to automatically fill each Y value with the most recent X value. Enabled by default once `x_axis` is set; usually no need to change.                                                                                                             |
...
A custom X axis is assumed to be **monotonically non-decreasing**: only the first Y point is kept for a given X value. With a non-monotonic X (e.g. 5→6→5), a rolled-back X value is accepted as new, so multiple Y points may appear at the same X value.
...
When `hidden` is not passed, the previous setting is kept; to undo a previous `hidden=True`, pass `hidden=False` explicitly (this also works in merge mode).
