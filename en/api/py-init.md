...
| mode            | (str) Sets the mode for creating SwanLab experiments. Options are "online", "local", "offline", or "disabled". Default is "online".<br>`online`: Uploads the experiment to the SwanLab cloud (public or private deployment).<br>`offline`: Only records experiment data locally.<br>`local`: Does not upload to the cloud but records experiment information locally.<br>`disabled`: Neither uploads nor records. |
...
| parallel        | (str) Parallel mode. Options are `"none"` or `"shared"`. Defaults to `"none"`.<br>`"shared"`: Shared parallel mode, allowing multiple distributed processes to upload metrics to the same experiment simultaneously. Automatically forces `mode='online'` and `resume='allow'`, and auto-generates an 8-character random `id` if none is provided.                                                                |
...
For more information about plugins, refer to the [Plugins](../plugin/plugin-index.md) documentation.
