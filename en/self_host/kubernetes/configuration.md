...
| `global.settings.host`                     | string | `""`            | Login host address. After modification, the API Key login address displayed in the frontend application will change accordingly (does not affect the actual backend service address). If you use SSO features, we strongly recommend setting it to the external URL of your gateway. This field is required for TRUSTED login. |
...
> 2. If you use the SSO feature, it is strongly recommended to set this field; otherwise, SwanLab attempts to infer the external address from the request information, which may lead to issues. You must set this field explicitly for TRUSTED login; otherwise, Auth cannot determine a reliable SwanLab post-login redirect URL.
